...
cmp al, NULL
je %%sort 			; end of string, goto convert
...
%%OK:
;mov byte [rbx+r12], al 	; unneeded
inc r12				; increment count
inc rsi				; advance through string
jmp %%readChr

%%sort:
;mov rax, 0
;mov al, byte [rbx+r13]
movzx rax, byte [%1 + rsi] 	; read string[i], using movzx to get rid of "mov rax,0"
cmp al, 'a'
jae %%intDigit1
cmp al, 'A'
jae %%intDigit2
...
movss xmm1, dword [t]
call cosf
mulss xmm1, dword [radii]
movss dword [xtmp], xmm1
...
movss xmm7, xmm5
call cosf
mulss xmm7, dword [ofp]

;[(r1+r2)*cos(t)] + [offPos * [cos[(r1+r2) * [(t+s)/(r2)]]]]
addss xmm7, dword [xtmp]
movss xmm0, xmm7
movss dword [x], xmm0
