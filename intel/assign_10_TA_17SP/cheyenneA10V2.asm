...
;ADD THIS
jmp %%NO  ;if it passes through and is not OK, then it should be NO
...
; push pop before/after macro
; to save your arguments and avoid segFault
push 	rsi
push	rdx
push 	rcx
push 	r8
...
mov bl, byte[r10] 		; get the first character of color param

cmp byte[r10+1], NULL		; if not ONLY one character
jne doCLvalueErr 		; print error
...
colorSet:
; Colors already set from your nxtCheck#, just call glColor3ub
; Moved so that you don't mess up rdi with the stuff down on line 652
;mov dil, byte [red]
;mov sil, byte [green]
;mov dl, byte [blue]
call glColor3ub
...
ucomiss xmm0, xmm1 ;change dword[limit] to xmm1
jl drawSpiroLp
